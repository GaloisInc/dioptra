...
def ct_pow_memo(cc: Analyzer, x: Ciphertext, y: int, memo: dict[tuple[Ciphertext, int], Ciphertext]) -> Ciphertext:
...
def ct_pow_memo_rec(cc: Analyzer, x: Ciphertext, y: int, memo: dict[tuple[Ciphertext, int], Ciphertext]) -> Ciphertext:
  assert(y > 0)
...
def pow_perf(cc: Analyzer, pow: Callable[[Analyzer, Ciphertext, int], Ciphertext]) -> None:
  x = cc.ArbitraryCT()
