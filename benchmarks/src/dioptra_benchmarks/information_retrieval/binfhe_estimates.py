...
def binfhe_pir_case(bits: int) -> Callable[[BinFHEAnalyzer], None]:
  def run_case(cc: BinFHEAnalyzer):
    db_size = 2 ** bits
    database = list(([x], x * x) for x in range(0, db_size))
    query = arbitrary_circuit(cc, bits)
    pir.pir_binfhe_lookup(database, [query])
...
def binfhe_pir_estimates(ec: EstimationCases):
  for bits in [4, 6, 8, 10]:
    ec.add_binfhe_case(binfhe_pir_case(bits), f"pir_binfhe_lookup [database size {2**bits}]")
