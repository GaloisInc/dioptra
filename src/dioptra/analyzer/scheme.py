...
class LevelInfo:
    ...
    def max(self, other: "LevelInfo") -> "LevelInfo":
        if self.level < other.level or (
            self.level == other.level and self.noise_scale_deg < other.noise_scale_deg
        ):
            return other
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"noise_scale_deg": self.noise_scale_deg, "level": self.level}
...
class SchemeModelPke:
    ...
    def min_level(self) -> LevelInfo:
        raise NotImplementedError(
            f"scheme '{self.name}' does not implement `min_level`"
        )

    def mul_level(self, lev1: LevelInfo, lev2: LevelInfo) -> LevelInfo:
        raise NotImplementedError(
            f"scheme '{self.name}' does not implement `mul_level`"
        )

    def add_level(self, lev1: LevelInfo, lev2: LevelInfo) -> LevelInfo:
        raise NotImplementedError(
            f"scheme '{self.name}' does not implement `add_level`"
        )

    def num_slots(self, cc: openfhe.CryptoContext) -> int:
        raise NotImplementedError(
            f"scheme '{self.name}' does not implement `num_slots`"
        )

    def arbitrary_pt(
        self, cc: openfhe.CryptoContext, level: LevelInfo
    ) -> openfhe.Plaintext:
        raise NotImplementedError(
            f"scheme '{self.name}' does not implement `arbitrary_ct`"
        )

    def arbitrary_ct(
        self, cc: openfhe.CryptoContext, sk: openfhe.PublicKey, level: LevelInfo
    ) -> openfhe.Ciphertext:
        return cc.Encrypt(sk, self.arbitrary_pt(cc, level))

    def bootstrap_level(self, lev: LevelInfo) -> LevelInfo:
        raise NotImplementedError(
            f"scheme '{self.name}' does not implement `bootstrap_effect`"
        )
...
class SchemeModelCKKS(SchemeModelPke):
    ...
    def arbitrary_pt(
        ...
        return cc.MakeCKKSPackedPlaintext(
            [0] * self.num_slots(cc), level=level.level, scaleDeg=level.noise_scale_deg
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"name": self.name, "bootstrap_level": self.bootstrap_lev.to_dict()}
...
class SchemeModelBGV(SchemeModelPke):
    ...
    def arbitrary_pt(
        ...
        return cc.MakePackedPlaintext(
            [0] * self.num_slots(cc), level=lv.level, noiseScaleDeg=lv.noise_scale_deg
        )
...
class SchemeModelBFV(SchemeModelPke):
    ...
    def arbitrary_pt(
        ...
        return cc.MakePackedPlaintext(
            [0] * self.num_slots(cc),
            level=level.level,
            noiseScaleDeg=level.noise_scale_deg,
        )
